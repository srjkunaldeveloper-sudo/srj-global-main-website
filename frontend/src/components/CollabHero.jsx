...
export default function CollabHero() {
  ...
  const collabCards = [
    { id: 'validation', title: 'Consultation', desc: 'Strategy & Planning', highlight: 'Idea Validation', detail: 'Market Research', posClass: 'top-left' },
    { id: 'development', title: 'Development', desc: 'Custom Software', highlight: 'Web + Mobile + AI', detail: 'Production Ready', posClass: 'top-right' },
    { id: 'support', title: 'Lifetime Support', desc: 'Growth & Scalability', highlight: 'Senior Engineers', detail: 'Dedicated Team', posClass: 'bottom-center' }
  ];
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Cards floating
      document.querySelectorAll('.collab-ecosystem-card').forEach((card, idx) => {
        gsap.to(card, {
          y: idx % 2 === 0 ? '-8px' : '8px',
      ...
      const handleMouseMove = (e) => {
        ...
        gsap.to('.collab-ecosystem-wrapper', {
          x: moveX,
          y: moveY,
          duration: 1,
          ease: 'power2.out'
    ...
    >
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8 items-center z-10">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-[54px] font-black text-black leading-[1.08] tracking-tight mb-6">
            <span className="block">
              {words.slice(0, 2).map((word, idx) => (
                <span key={idx} className="collab-hero-word inline-block mr-3 opacity-0 select-none">{word}</span>
              ))}
            </span>
            <span className="block mt-1">
              {words.slice(2, 3).map((word, idx) => (
                <span key={idx} className="collab-hero-word inline-block mr-3 opacity-0 select-none">{word}</span>
              ))}
            </span>
            <span className="block mt-1">
              {words.slice(3).map((word, idx) => (
                <span key={idx} className="collab-hero-word inline-block mr-3 opacity-0 select-none">{word}</span>
              ))}
            </span>
          </h1>
        ...
        <div className="lg:col-span-7 flex justify-center lg:justify-end items-center relative min-h-[500px] w-full select-none lg:pr-6">
          <div className="collab-ecosystem-wrapper relative w-[320px] sm:w-[500px] h-[320px] sm:h-[500px] flex items-center justify-center">
            ...
            {/* Collab Cards */}
            {collabCards.map((card, index) => (
              <div 
                key={card.id}
                onMouseEnter={(e) => handleMouseEnter(e, index)}
                onMouseLeave={(e) => handleMouseLeave(e, index)}
                className={`collab-ecosystem-card absolute bg-white border border-slate-200 p-4 sm:p-5 rounded-[20px] shadow-sm transition-all duration-350 z-20 w-[125px] sm:w-[150px] hover:z-30 text-left cursor-pointer
