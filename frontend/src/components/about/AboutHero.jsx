...
export default function AboutHero() {
  ...
  const headingText = "We Build Products That Drive Results.";
  ...
  const aboutCards = [
    { id: 'transform', title: 'Transform', desc: 'From Concept to Product', highlight: 'Ideas to Code', detail: 'Agile Delivery', posClass: 'top-left' },
    { id: 'scale', title: 'Scale', desc: 'Secure & Modular', highlight: 'Robust Architecture', detail: 'Cloud Infrastructure', posClass: 'top-right' },
    { id: 'deliver', title: 'Deliver', desc: 'Quality Software', highlight: 'End-to-End Engineering', detail: 'Production Ready', posClass: 'bottom-center' }
  ];
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Cards floating
      document.querySelectorAll('.about-ecosystem-card').forEach((card, idx) => {
        gsap.to(card, {
          y: idx % 2 === 0 ? '-8px' : '8px',
      ...
      const handleMouseMove = (e) => {
        ...
        gsap.to('.about-ecosystem-wrapper', {
          x: moveX,
          y: moveY,
          duration: 1,
          ease: 'power2.out'
      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8 items-center z-10">
        ...
        <div ref={leftSideRef} className="lg:col-span-5 flex flex-col justify-center text-left">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-[54px] font-black text-black leading-[1.08] tracking-tight mb-6">
            <span className="block">
              {words.slice(0, 2).map((word, idx) => (
                <span key={idx} className="about-hero-word inline-block mr-3 opacity-0 select-none">{word}</span>
              ))}
            </span>
          ...
          <p className="left-fade-rest text-slate-500 text-sm sm:text-base leading-relaxed mb-10 max-w-lg">
            SRJ Global Technologies is a full-service technology company dedicated to helping businesses transform their ideas into powerful digital solutions. From web and mobile development to AI, cloud, and design — we deliver end-to-end engineering that scales.
          </p>
          ...
          <div className="left-fade-rest grid grid-cols-2 gap-y-3.5 gap-x-4 max-w-md border-t border-slate-100 pt-8">
            <div className="flex items-center gap-2 text-black text-xs font-semibold">
              ...
              <span>Full-Service Engineering</span>
            </div>
            <div className="flex items-center gap-2 text-black text-xs font-semibold">
              <Check size={14} className="text-slate-800 flex-shrink-0" />
            ...
            <div className="flex items-center gap-2 text-black text-xs font-semibold">
              <Check size={14} className="text-slate-800 flex-shrink-0" />
              <span>Scale-Focused Approach</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 flex justify-center lg:justify-end items-center relative min-h-[500px] w-full select-none lg:pr-6">
          <div className="about-ecosystem-wrapper relative w-[320px] sm:w-[500px] h-[320px] sm:h-[500px] flex items-center justify-center">
            ...
            {aboutCards.map((card, index) => (
              <div 
                ...
                className={`about-ecosystem-card absolute bg-white border border-slate-200 p-4 sm:p-5 rounded-[20px] shadow-sm transition-all duration-350 z-20 w-[125px] sm:w-[150px] hover:z-30 text-left cursor-pointer
